...
`viewBox` で寸法を決める。`width`・`height` は `viewBox` と同じ数値を指定し、埋め込み先で拡大縮小できるようにする。箱の左上・幅・高さは 20px 単位のグリッドにそろえる。辺の端点はマーカーの余白分(2px 程度)を引いてよい。基準線は箱の中央からフォントサイズの約 3 分の 1 だけ下げる。
