...
| 近黒の背景にアクセントカラー 1 色 | 配色は 2. の色スケールから、題材に合わせて選ぶ |
| 中央寄せの hero に CTA 1 つ、誰の製品にも当てはまる見出し | 見出しは題材固有の一文にする(`references/text.md`) |
...
- モーションは `prefers-reduced-motion` を尊重する(`references/checklist.md`)。
...
- 感嘆符を多用しない。エラー・空状態は謝罪や煽りでなく、事実と次の行動を書く(`references/text.md`)。
