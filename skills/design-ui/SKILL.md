---
name: design-ui
description: Web UI(画面・コンポーネント・ページ)を設計・実装するときの規範。題材から出発してトークン案を書き、汎用の既定と照合してから実装し、有限回の検証で止める工程を定める。「画面を作って」「UI を実装して」「デザインして」「もっと良くして」「AI っぽい」と言われたとき、色・書体・間隔・階層・状態を決めるときに使う。
---
