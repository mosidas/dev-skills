---
name: design-ui
description: Web・iOS・Android・デスクトップの画面・コンポーネント・ページを設計・実装するときの規範。題材から出発してトークン案を書き、汎用の既定と照合してから実装し、有限回の検証で止める工程を定める。「画面を作って」「UI を実装して」「デザインして」「もっと良くして」「画面が AI っぽい」「アプリの画面を作って」「モバイルアプリを実装して」と言われたとき、色・書体・間隔・階層・状態を決めるときに使う。
---
...
- 適用範囲は Web・iOS・Android・デスクトップの画面・コンポーネント・ページの設計と実装。
...
- 優先順位は次のとおりとする。上位が下位に優先する。
  ...
  3. OS の規約(Apple Human Interface Guidelines・Material Design 3)。ネイティブアプリを設計・実装するときに従う。
  4. 本スキルの規範。
...
4. 実装する。文言の配置と分量は `references/text.md` に、アクセシビリティ・フォーカス・フォーム・モーション・状態など実装の品質床(共通と Web 固有の節に分かれる)は `references/checklist.md` に、それぞれ従う。
...
| `references/checklist.md` | 実装の品質床(共通の節と Web 固有の節。アクセシビリティ・フォーカス・フォーム・モーション・状態) | 実装し、検証するとき |
| `references/native.md` | ネイティブアプリ固有の規範(セーフエリア・文字の拡縮・タップ領域・ナビゲーション) | ネイティブアプリ・モバイルアプリを設計・実装するとき |
