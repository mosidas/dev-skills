...
def plugin_root() -> Path:
    """プラグインのルートディレクトリ。環境変数を第一、ファイル位置を第二とする。"""
    env = os.environ.get(PLUGIN_ROOT_ENV)
    return Path(env) if env else DEFAULT_PLUGIN_ROOT
...
def lint_command() -> list[str] | None:
    ...
    if override:
        try:
            ...
            return None
    lint_py = plugin_root() / LINT_REL
    if not lint_py.is_file():
        return None
    return ["uv", "run", str(lint_py)]


def run_lint(path: Path, genre: str | None, config: dict) -> list[dict] | None:
...
def load_phrase_okays() -> dict[str, list[str]]:
    """NG/OK カタログから「語 → OK 言い換え例」の対応を読む(読めなければ空)。"""
    catalog = _load_json(plugin_root() / PHRASE_CATALOG_REL)
