...
出典: coji/natural-japanese (MIT, https://github.com/coji/natural-japanese) から移植。
    コメント中の HANDOFF.md・corpus/ 等は原リポジトリ内の較正資料を指す
    (校正の経緯の記録として残している。本スキルには含まれない)。
    検査の手順・カテゴリごとの対応方針は references/inspection.md を参照。
...
# 経緯は references/sentence.md の翻訳調(連体修飾の入れ子)の節にも記載。
