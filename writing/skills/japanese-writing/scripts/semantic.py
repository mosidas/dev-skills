...
依存するため、lint.py にも既定の検査ループにも組み込まない。実行したい人だけが
明示的に `uv run .claude/skills/japanese-writing/scripts/semantic.py` を叩く。
使いどころは `references/inspection.md` 2.4 に従う。

背景・設計思想(原典 coji/natural-japanese の corpus/reports/nn-detector-sweep.md §B 参照):
...
指標定義(原典の corpus/experiments/embedding/sweep.py と同一ロジック。すべて cos類似度、
...
文分割は原典のコーパス実験(corpus/experiments/embedding/embed_corpus.py)と同じ経路
...
# ---------------------------------------------------------------------------
# 閾値校正(2026-07、原典 coji/natural-japanese の corpus/experiments/embedding/
# sweep-raw.json 542文書分の生データを原典スクリプト作成時に再集計して算出。
# 算出手順は corpus/experiments/embedding/sweep.py の sweep_threshold() と同じ規律
...
#     (原典 corpus/reports/nn-detector-sweep.md §B の実験結果そのまま)
#   - ジャンル別再校正(原典スクリプト作成時に実施。essayのFP率問題への対応):
...
# (原典 corpus/experiments/embedding/sweep-result.md の全体スイープ結果をそのまま流用。
...
def doc_sentences_with_lines(raw_text: str) -> list[tuple[int, str]]:
    """(行番号, 原文の文) のリストを返す。原典 embed_corpus.py の doc_sentences と
...
def compute_metrics(embeddings) -> dict:
    """原典 corpus/experiments/embedding/sweep.py の cosine_matrix_stats と同一ロジック。"""
    import numpy as np
        ...
        findings.append(
            Finding(
                ...
                detail=(
                    ...
                    f"{flatness_calibration}(原典 coji/natural-japanese の "
                    "corpus/experiments/embedding/sweep-raw.json 542文書からの実測。"
                    "genre指定なしは原典 corpus/reports/nn-detector-sweep.md "
        ...
        findings.append(
            Finding(
                ...
                detail=(
                    ...
                    "AI検出率=46.4%(原典 corpus/experiments/embedding/sweep-result.md)。"
                ),
            )
        )
        ...
        findings.append(
            Finding(
                ...
                detail=(
                    ...
                    "(原典 corpus/experiments/embedding/sweep-result.md)。"
                ),
            )
        )
